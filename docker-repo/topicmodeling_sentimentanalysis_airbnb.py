# -*- coding: utf-8 -*-
"""TopicModeling_SentimentAnalysis_LDA_using_Scikit_learn.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/github/sajaldebnath/topic_sentiment_analysis/blob/main/TopicModeling_SentimentAnalysis_LDA_using_Scikit_learn.ipynb
...
def preprocess_texts(comments):
    ...
    urlPattern = (
        r"http[s]?://\S+"  # Removes any website link starting with http or https
    )
    ...
    seqReplacePattern = (
        r"\1\1"  # Replace the repeating letter sequence with two letters
    )
...
t = time.time()
data["processed_comments"] = data["comments"].apply(
    preprocess_texts
)  # applying the preprocess_comments function to comments column
...
)  # To view text column completely and so that pandas doesn't truncate the text
    ...
    for word in words:
        if word not in stop_words:
            ...
            word = lemmatizer.lemmatize(
                word, pos="v"
            )  # Lemmatize as a verb, pos='v' ensures that
...
data["sentiment"] = data["comments"].apply(
    lambda x: sentiment_intensity_analyzer.polarity_scores(x)["compound"]
)
...
# print(topics)
# Print sentiment scores for each topic
for topic_idx, sentiment_score in topic_sentiments.items():
    print(f"Topic {topic_idx + 1} Sentiment Score: {sentiment_score:.2f}")
    print(f"Top Words: {topics[topic_idx]}\n")
...
def coherence_score(model, feature_names, X):
    ...
    for topic in topic_word_distributions:
        ...
        pairwise_sims = 1 - pairwise_distances(
            doc_term_matrix[:, top_words_idx].T, metric="cosine"
        )
...
"""### Hypertuning of LDA using scikit-learn

The **param_grid** dictionary in hyperparameter tuning for the Latent Dirichlet Allocation (LDA) model defines the set of hyperparameters and their respective values to be tested during the tuning process. Here’s a breakdown of the components in the param_grid and their significance:

**Components in param_grid**

1. **n_components (Number of Topics)**
...
* Adjust n_components based on domain knowledge (e.g., if analyzing customer reviews, a smaller number of topics like 3–5 may suffice).

* Experiment with different learning_decay and learning_offset values to see their impact on coherence and stability of topics.
"""
...
def calculate_coherence(model, doc_term_matrix):
    ...
    assigned_topics = topic_distribution.argmax(
        axis=1
    )  # Assign the most probable topic for each doc
...
for n_topics in tqdm(n_components_range, desc="Tuning n_components"):
    for decay in learning_decay_range:
        for offset in learning_offset_range:
            ...
            print(
                f"n_topics: {n_topics}, learning_decay: {decay}, learning_offset: {offset}, score: {score:.4f}"
            )
...
print("\nBest Parameters:")
print(
    f"n_topics: {best_params['n_topics']}, learning_decay: {best_params['learning_decay']}, learning_offset: {best_params['learning_offset']}"
)
...
# Assign updated topics to reviews
topic_assignments = best_model.transform(doc_term_matrix)
data["topic"] = topic_assignments.argmax(axis=1)
...
sns.boxplot(
    data=data, x="topic", hue="topic", y="sentiment", palette="coolwarm", legend=False
)
...
topic_assignments = best_model.transform(doc_term_matrix).argmax(axis=1)
data["topic"] = topic_assignments

# Plot sentiment by topic
sentiment_by_topic = (
    data.groupby("topic")["sentiment_label"].value_counts(normalize=True).unstack()
)
...
lda_vis = pyLDAvis.lda_model.prepare(best_model, doc_term_matrix, vectorizer)
...
def test_preprocess_texts():
    ...
    result = preprocess_texts(
        '"This is a great product. I love it!", "The product is okay, but could be improved.", "Terrible experience. I hate it.", "Amazing quality and excellent customer service."'
    )
